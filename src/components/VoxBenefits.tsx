...
function BenefitCard({
  ...
  // ✅ Hook é chamado no topo do componente (não dentro de callback/loop)
  const cardRef = useScrollAnimation<HTMLDivElement>({ delay });
...
export const VoxBenefits = () => {
  ...
  const content = {
    ...
    benefits_subtitle:
      "Dois dias de imersão presencial com acesso exclusivo a:",
  ...
  const benefits: BenefitItem[] = [
    {
      icon: CheckCircle,
      text: "Workshops de alto impacto com resultados aplicáveis ao seu negócio",
    },
    {
      icon: Users,
      text: "Networking estratégico com líderes e empresários que impulsionam a região",
    },
    { icon: BarChart, text: "Painéis com especialistas de mercado e gestão" },
    {
      icon: TrendingUp,
      text: "Os 4 pilares que sustentam empresas de alto nível: Gestão • Estratégia • Cultura • Finanças",
    },
    {
      icon: DollarSign,
      text: "Transformação empresarial: eficiência, previsibilidade e independência para decisões seguras",
    },
    {
      icon: Coffee,
      text: "Experiência completa: coffee break, almoço executivo e happy hour",
    },
  ];
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-16">
          {benefits.map((benefit, index) => (
            <BenefitCard
              key={benefit.text} // lista estática: usar o texto é ok aqui
              icon={benefit.icon}
              text={benefit.text}
              delay={index * 100}
            />
